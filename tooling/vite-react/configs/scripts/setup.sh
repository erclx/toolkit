...
configure_identity() {
  ask "Project Name?" "RAW_NAME" "my-project"

  [ -z "$RAW_NAME" ] && log_error "Project name cannot be empty"
...
update_html_title() {
  if [ -f "index.html" ]; then
    ...
    node -e '
      const fs = require("fs");
      const title = process.env.TITLE || "App";
      let html = fs.readFileSync("index.html", "utf8");
      html = html.replace(/<title[^>]*>.*?<\/title>/, "<title>" + title + "</title>");
      fs.writeFileSync("index.html", html);
    ' || log_error "index.html update failed"
    log_add "index.html"
  fi
}
