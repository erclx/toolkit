...
A concise overview of all documents in the dev workflow,
with placeholders and tool mappings.

> **Session note:** "Session start" = opening a new chat tab (new context window).
> New message in the same chat = no re-orientation needed.
> New chat tab = paste in this order: `CLAUDE.md` → `TASKS.md` → active feature file.
> Add `REQUIREMENTS.md` + `ARCHITECTURE.md` only when starting a brand new feature.
...
> Auto-generated by `bun run snapshot`. Refresh when structure meaningfully changes.
> Automatically generated and updated as part of `bun run check` to ensure an up-to-date project context.
> Paste with REQUIREMENTS.md + ARCHITECTURE.md on new feature sessions only.
...
> Created with: **Claude chat**
> One-time spike before writing any code.
...
> Created with: **Claude chat**, updated manually or via Claude each session.
> This is your source of truth for what's done, in progress, blocked.
...
> Created with: **Claude chat** at the start of each feature.
> Move to `FEATURES/done/` when feature is committed.
